...
def _result_from_payload(
    ...
    *,
    ...
    return MessageUnderstandingResult(
        ...
        user_needs=specificity["user_needs"],
        territory=territory,
        requested_channel=requested_channel,
...
def _normalize_specificity(
    *,
    ...
    needs_service_discovery: bool,
) -> dict[str, Any]:
    """Conservative post-processing for LLM dispatcher slots.

    The model is useful for extracting semantic slots, but it sometimes marks
    specific eligibility questions as broad service discovery. This helper does
    not create legal facts and does not choose a service. It only:
    - preserves obvious facts/needs already present in the user's text;
    - prevents broad-discovery routing when the message contains a concrete
      applicant/situation + concrete need bundle.
    """

    question = _normalize_for_match(normalized_question)
    facts = list(applicant_facts or [])
    needs = list(user_needs or [])

    def add_fact(value: str) -> None:
        _append_unique(facts, value)

    def add_need(value: str) -> None:
        _append_unique(needs, value)

    if _contains_all(question, ("нечего", "есть")) or "нет еды" in question:
        add_fact("нет еды")
        add_need("питание")

    if "продукт" in question:
        add_need("продукты")

    if "дет" in question and "школ" in question:
        add_fact("дети школьного возраста")
        add_need("подготовка к школе")

    if ("ребен" in question or "ребён" in question) and "инвалид" in question:
        add_fact("ребенок-инвалид")
    if "коляск" in question:
        add_need("кресло-коляска")
    if "слухов" in question and "аппарат" in question:
        add_need("слуховой аппарат")

    if "павод" in question:
        add_fact("паводок")
    if "чрезвычай" in question or " чс" in f" {question} ":
        add_fact("чрезвычайная ситуация")
    if "пострадавш" in question:
        add_fact("пострадавший")
    if "имуществ" in question:
        add_fact("имущество")
    if "материальн" in question and "помощ" in question:
        add_need("материальная помощь")
    if "выплат" in question:
        add_need("выплата")

    if _has_transport_need(question):
        add_need("проезд")
    if "лечение" in question:
        add_need("лечение")
    if "жку" in question or "коммунал" in question:
        add_need("компенсация ЖКУ")
    if "льготник" in question:
        add_fact("льготник")

    if _is_broad_service_discovery_question(
        question=question,
        intent=intent,
        service_hint=service_hint,
        topic=topic,
        ...
        needs_service_discovery = True
    elif _is_specific_eligibility_bundle(
        question=question,
        intent=intent,
        service_hint=service_hint,
        topic=topic,
        ...
        needs_service_discovery = False

    return {
        "applicant_facts": facts[:12],
        "user_needs": needs[:12],
        "needs_service_discovery": needs_service_discovery,
    }
...
def _is_broad_service_discovery_question(
    *,
    question: str,
    intent: str,
    service_hint: Optional[str],
    ...
    """Return True for broad life-situation questions.

    Step43 made the post-processor too eager: any fact plus any need could
    become a "specific" eligibility bundle. As a result, broad questions like
    "what am I entitled to" or "help, my house burned down" stopped going to
    service_discovery. This helper does not choose a service. It only preserves
    the broad-discovery route when the user expects the system to propose
    possible measures.
    """
    ...
    if service_hint and not _contains_any_phrase(question, ("что мне положено", "что положено", "какие меры", "меры поддержки")):
        return False

    exact_specific_patterns = (
        _contains_any_token(question, ("проезд", "поездк", "ехать", "дорог", "добраться")) and "лечение" in question,
        ("ребен" in question or "ребён" in question) and "инвалид" in question and _contains_any_token(question, ("коляск", "слухов", "аппарат")),
        _contains_any_token(question, ("павод", "чс", "чрезвычай")) and "имуществ" in question,
        _contains_any_token(question, ("чс", "чрезвычай")) and "материальн" in question and "помощ" in question,
        _contains_any_token(question, ("жку", "коммунал")) and bool(territory or "эвенк" in question),
    )
    if any(exact_specific_patterns):
        return False
    ...
    if _contains_any_phrase(question, broad_phrases):
        return True
    ...
    if _contains_any_phrase(question, vague_help_phrases):
        return True

    facts_text = _normalize_for_match(" ".join(facts or []))
    needs_text = _normalize_for_match(" ".join(needs or []))
    topic_text = _normalize_for_match(topic or "")

    combined = " ".join([question, facts_text, needs_text, topic_text])
    broad_situation_tokens = (
    ...
    if _contains_any_phrase(combined, broad_situation_tokens):
        return True

    if "школ" in question and _contains_any_phrase(question, ("помощ", "не хватает денег", "собрать")):
        return True

    return False

def _is_specific_eligibility_bundle(
    *,
    question: str,
    intent: str,
    service_hint: Optional[str],
    ...
    has_fact = bool(facts_text)
    has_need = bool(needs_text)
    has_context = bool(territory or requested_channel or topic_text)
    ...
    if any(exact_patterns):
        return True

    return has_need and (has_fact or has_context)


def _has_transport_need(question: str) -> bool:
    return _contains_any_token(question, ("проезд", "поездк", "ехать", "добраться", "дорог", "направляют", "отправляют"))
...
def _append_unique(values: list[str], value: str) -> None:
    ...
    if normalized in {_normalize_for_match(item) for item in values}:
        return
    values.append(text)


def _contains_all(value: str, parts: tuple[str, ...]) -> bool:
    return all(part in value for part in parts)


def _contains_any_token(value: str, stems: tuple[str, ...]) -> bool:
    return any(stem in value for stem in stems)


def _contains_any_phrase(value: str, phrases: tuple[str, ...]) -> bool:
    normalized_value = _normalize_for_match(value)
    return any(_normalize_for_match(phrase) in normalized_value for phrase in phrases if phrase)
