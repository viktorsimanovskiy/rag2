...
>>>>>>> 4c04853102b91a0c6e1fdd3692b3fb98688e2646
    ...
    # устойчивые ложные высокоуверенные совпадения, где общий термин давал
    # score=100 не той услуге.

    oncology_care_question_v2 = (
        q_text_has_any({"онколог", "онкологи", "онкобольн"})
        and q_text_has_any({"материальн", "уход", "одинок", "одинокопрожив"})
    )
    oncology_care_service_v2 = (
        service_text_has_any({"онколог", "онкологи", "онкобольн"})
        and service_text_has_any({"материальн", "уход"})
    )
    if oncology_care_question_v2:
        if oncology_care_service_v2:
            adjusted += 76.0
            accumulator.match_reasons.append("усиление: уход/материальная помощь при онкологии")
        elif service_text_has_any({"соцобслуживан", "социальном обслуживании", "социального обслуживания"}):
            adjusted = min(adjusted * 0.18, 46.0)
            accumulator.match_reasons.append(
                "сильное понижение: вопрос про материальную помощь при онкологии, не общее соцобслуживание"
            )

    low_income_child_travel_question_v2 = (
        question_has_child_context
        and q_text_has_any({"малообеспеч", "малобеспеч", "малоимущ"})
        and q_text_has_any({"проезд", "компенсац", "билет", "дорог", "сопровождающ"})
    )
    low_income_child_travel_service_v2 = (
        service_has_child_context
        and service_text_has_any({"малообеспеч", "малобеспеч", "малоимущ"})
        and service_text_has_any({"проезд", "сопровождающ", "компенсац"})
    )
    if low_income_child_travel_question_v2:
        if low_income_child_travel_service_v2:
            adjusted += 76.0
            accumulator.match_reasons.append("усиление: компенсация проезда ребёнка из малообеспеченной семьи")
        elif service_text_has_any({"родители являются инвалидами", "родителей являются инвалидами", "родитель инвалид"}):
            adjusted = min(adjusted * 0.20, 44.0)
            accumulator.match_reasons.append(
                "сильное понижение: вопрос про проезд ребёнка из малообеспеченной семьи, не пособие семьям инвалидов"
            )

    honorary_title_question_v2 = q_has_any({"почетн"}) and q_has_any({"гражданин", "жител"})
    honorary_citizen_question_v2 = q_has_any({"почетн"}) and q_has_any({"гражданин"})
    honorary_resident_question_v2 = q_has_any({"почетн"}) and q_has_any({"жител"})
    if honorary_title_question_v2:
        service_has_honorary = service_text_has_any({"почетн"})
        service_has_honorary_citizen = service_has_honorary and service_text_has_any({"граждан"})
        service_has_honorary_resident = service_has_honorary and service_text_has_any({"жител"})
        if honorary_citizen_question_v2 and service_has_honorary_citizen:
            adjusted += 68.0
            accumulator.match_reasons.append("усиление: почётный гражданин")
        elif honorary_resident_question_v2 and service_has_honorary_resident:
            adjusted += 68.0
            accumulator.match_reasons.append("усиление: почётный житель")
        elif not service_has_honorary:
            adjusted = min(adjusted * 0.20, 52.0)
            accumulator.match_reasons.append(
                "сильное понижение: вопрос про почётное звание, услуга без признака почётного гражданина/жителя"
            )
        elif honorary_citizen_question_v2 and not service_has_honorary_citizen:
            adjusted = min(adjusted * 0.52, 70.0)
            accumulator.match_reasons.append("понижение: вопрос про почётного гражданина, услуга про другой почётный статус")
        elif honorary_resident_question_v2 and not service_has_honorary_resident:
            adjusted = min(adjusted * 0.52, 70.0)
            accumulator.match_reasons.append("понижение: вопрос про почётного жителя, услуга про другой почётный статус")


    # second_step_32: точечная донастройка после проверки second_step_31.
    #
    # 1) В runtime_vocabulary фраза про «почётного гражданина/жителя»
    #    расширяет вопрос до обоих терминов сразу. Для развилки по почётному
    #    статусу используем raw-нормализацию вопроса без runtime-expansion,
    #    иначе «почётный гражданин» может выбрать услугу про «почётного
    #    жителя».
    # 2) По проезду ребёнка из малообеспеченной семьи первый кандидат уже
    #    правильный, но соседние детские/проездные меры оставались слишком
    #    близко и давали ambiguous. Здесь понижаем похожий, но не точный
    #    проездной шум.

    low_income_child_travel_question_v3 = (
        q_raw_has_any({"ребен", "дет"})
        and q_raw_has_any({"малообеспеч", "малобеспеч", "малоимущ"})
        and q_raw_has_any({"проезд", "компенсац", "билет", "дорог", "сопровождающ"})
    )
    low_income_child_travel_service_v3 = (
        service_text_has_any({"ребен", "дет"})
        and service_text_has_any({"малообеспеч", "малобеспеч", "малоимущ"})
        and service_text_has_any({"проезд", "компенсац", "сопровождающ"})
    )
    if low_income_child_travel_question_v3:
        if low_income_child_travel_service_v3:
            adjusted += 90.0
            accumulator.match_reasons.append(
                "усиление: точный проезд ребёнка из малообеспеченной семьи"
            )
        elif (
            service_has_child_context
            or service_text_has_any({
                "проезд",
                "путевк",
                "лагер",
                "социальн карт",
            ...
            adjusted = min(adjusted * 0.24, 58.0)
            accumulator.match_reasons.append(
                "сильное понижение: вопрос про проезд ребёнка из малообеспеченной семьи, другая детская/проездная мера"
            )

    raw_honorary_question = q_raw_has_any({"почетн"})
    raw_honorary_citizen_question = raw_honorary_question and q_raw_has_any({"граждан"}) and not q_raw_has_any({"жител"})
    raw_honorary_resident_question = raw_honorary_question and q_raw_has_any({"жител"}) and not q_raw_has_any({"граждан"})
    service_is_honorary_citizen_payment = (
        service_text_has_any({"почетн"})
        and service_text_has_any({"граждан"})
        and service_text_has_any({"ежемесячн", "едв", "выплат"})
        and not service_text_has_any({"погреб", "памятник", "похорон"})
    )
    service_is_honorary_resident_payment = (
        service_text_has_any({"почетн"})
        and service_text_has_any({"жител"})
        and service_text_has_any({"ежемесячн", "едв", "выплат"})
        and not service_text_has_any({"погреб", "памятник", "похорон"})
    )
    if raw_honorary_citizen_question:
        if service_is_honorary_citizen_payment:
            adjusted += 92.0
            accumulator.match_reasons.append("усиление: raw-вопрос про почётного гражданина")
        elif service_is_honorary_resident_payment:
            adjusted = min(adjusted * 0.16, 48.0)
            accumulator.match_reasons.append(
                "сильное понижение: raw-вопрос про почётного гражданина, не почётного жителя"
            )
        elif service_text_has_any({"погреб", "памятник", "похорон"}) and not burial_question:
            adjusted = min(adjusted * 0.20, 52.0)
            accumulator.match_reasons.append(
                "сильное понижение: вопрос про выплату почётному гражданину, не погребение/памятник"
            )

    if raw_honorary_resident_question:
        if service_is_honorary_resident_payment:
            adjusted += 92.0
            accumulator.match_reasons.append("усиление: raw-вопрос про почётного жителя")
        elif service_is_honorary_citizen_payment:
            adjusted = min(adjusted * 0.16, 48.0)
            accumulator.match_reasons.append(
                "сильное понижение: raw-вопрос про почётного жителя, не почётного гражданина"
            )
        elif service_text_has_any({"погреб", "памятник", "похорон"}):
            adjusted = min(adjusted * 0.20, 52.0)
            accumulator.match_reasons.append(
                "сильное понижение: вопрос про выплату почётному жителю, не погребение/памятник"
            )


    # second_step_33/34: развилки на уровне краткого наименования услуги.
    #
    # Проверка second_step_32 показала, что service_text слишком широкий:
    # в полное описание и алиасы могут попадать общие слова "граждане",
    # "жители", "проезд", из-за чего разные близкие услуги снова получают
    # score=100. Здесь для нескольких устойчивых конфликтов используем
    # более узкий сигнал — service_name_short.
    #
    # second_step_34: service_name_short нужно нормализовать БЕЗ runtime-
    # expansion. Иначе runtime_vocabulary расширяет сам title услуги
    # "почётный житель" до связки "почётный гражданин / почётный житель",
    # и title-level развилка опять не может отличить две разные услуги.
    service_title_text = _normalize_text_without_runtime_expansion(
        service_doc.service_name_short or ""
    )

    def service_title_has_any(markers: tuple[str, ...] | set[str]) -> bool:
        return any(marker in service_title_text for marker in markers)

    title_is_low_income_child_travel = (
        service_title_has_any({"компенсац"})
        and service_title_has_any({"проезд"})
        and service_title_has_any({"дет"})
        and service_title_has_any({"малообеспеч", "малобеспеч", "малоимущ"})
    )
    if low_income_child_travel_question_v3:
        if title_is_low_income_child_travel:
            adjusted = max(adjusted + 120.0, 130.0)
            accumulator.match_reasons.append(
                "усиление: краткое наименование точно про проезд ребёнка из малообеспеченной семьи"
            )
        elif service_title_has_any({"путевк", "лагер", "социальн карт", "погибш", "защитник", "пенси"}):
            adjusted = min(adjusted * 0.10, 40.0)
            accumulator.match_reasons.append(
                "сильное понижение: краткое наименование другой детской/проездной услуги"
            )

    # second_step_35: в кратких названиях аббревиатура "ЭДВ"
    # нормализуется как "эдв" (через "э"), а не как "едв".
    # Из-за этого title-level развилка second_step_34 не распознавала
    # краткие названия "ЕДВ почетным гражданам..." / "ЭДВ почетным
    # жителям..." как выплатные и не понижала соседний почётный статус.
    service_title_has_payment_marker = service_title_has_any({
        "едв",
        "эдв",
        "ежемесячн",
        "денежн",
    ...
    title_is_honorary_citizen_payment = (
        service_title_has_any({"почетн"})
        and service_title_has_any({"граждан"})
        and service_title_has_payment_marker
        and not service_title_has_any({"жител", "погреб", "памятник", "похорон"})
    )
    title_is_honorary_resident_payment = (
        service_title_has_any({"почетн"})
        and service_title_has_any({"жител"})
        and service_title_has_payment_marker
        and not service_title_has_any({"граждан", "погреб", "памятник", "похорон"})
    )
    if raw_honorary_citizen_question:
        if title_is_honorary_citizen_payment:
            adjusted = max(adjusted + 120.0, 130.0)
            accumulator.match_reasons.append("усиление: краткое наименование про ЕДВ почётному гражданину")
        elif title_is_honorary_resident_payment:
            adjusted = min(adjusted * 0.08, 34.0)
            accumulator.match_reasons.append(
                "сильное понижение: краткое наименование про почётного жителя, а вопрос про гражданина"
            )

    if raw_honorary_resident_question:
        if title_is_honorary_resident_payment:
            adjusted = max(adjusted + 120.0, 130.0)
            accumulator.match_reasons.append("усиление: краткое наименование про ЕДВ почётному жителю")
        elif title_is_honorary_citizen_payment:
            adjusted = min(adjusted * 0.08, 34.0)
            accumulator.match_reasons.append(
                "сильное понижение: краткое наименование про почётного гражданина, а вопрос про жителя"
            )


    # step39_practical_ambiguity_ranking_v2
    # --------------------------------------------------------
    # В step38 часть проверок опиралась на question_text после расширения
    # runtime_vocabulary. Для "материальная помощь при ЧС" словарь добавлял
    # соседние слова "утрата имущества", поэтому вопрос ошибочно попадал в
    # ветку имущественной ЧС-выплаты. Узкие развилки ниже работают по raw
    # вопросу, а не по расширенному тексту; расширения остаются полезными для
    # общего поиска кандидатов, но не управляют выбором между близкими ЧС-мерами.
    explicit_chs_question_v2 = q_raw_has_any({"чс", "чрезвычайн"})
    chs_material_help_question_v2 = (
        explicit_chs_question_v2
        and q_raw_text_has_any({"материальн", "матпомощ", "помощ"})
        and q_raw_text_has_any({"пострадав", "чрезвычайн", "чс"})
        and not q_raw_text_has_any({"утрат", "утрачен", "имущество", "имуществ", "вред", "здоров", "погиб", "умерш", "смерт"})
    )
    chs_property_question_v2 = explicit_chs_question_v2 and q_raw_text_has_any({
        "утрат",
        "утрачен",
        "имущество",
        "имуществ",
    ...
    chs_health_question_v2 = explicit_chs_question_v2 and q_raw_text_has_any({
        "вред здоров",
        "здоровью",
        "травм",
        "ранен",
    })
    chs_death_question_v2 = explicit_chs_question_v2 and q_raw_text_has_any({
        "погиб",
        "умерш",
        "смерт",
        "семьи погиб",
    ...
    title_is_chs_property = service_title_has_any({"утрат", "имуществ"})
    title_is_chs_health = service_title_has_any({"вред", "здоров", "травм"})
    title_is_chs_death = service_title_has_any({"погиб", "умерш", "смерт", "член", "сем"}) and explicit_chs_question_v2
    title_is_chs_material_help = (
        service_title_has_any({"материальн", "помощ", "пострадав"})
        and service_title_has_any({"чрезвычайн", "чс"})
        and not title_is_chs_property
        and not title_is_chs_health
        and not title_is_chs_death
    )
    title_is_tjs_material_help = service_title_has_any({"трудн", "жизненн", "тжс"})
    title_is_fire_safety_repair = (
        service_title_has_any({"печн", "отоплен", "электропровод", "пожарн", "безопасн"})
        and service_title_has_any({"ремонт", "помощ"})
    )

    if explicit_chs_question_v2 and title_is_fire_safety_repair and not q_raw_text_has_any({"печн", "отоплен", "электропровод", "ремонт", "пожарн", "безопасн"}):
        adjusted = min(adjusted * 0.12, 42.0)
        accumulator.match_reasons.append("step39 сильное понижение: ЧС-вопрос не про ремонт печного отопления")

    if chs_material_help_question_v2:
        if title_is_chs_material_help:
            adjusted = max(adjusted + 150.0, 140.0)
            accumulator.match_reasons.append("step39 усиление: материальная помощь пострадавшим при ЧС")
        elif title_is_chs_property or title_is_chs_health or title_is_chs_death:
            adjusted = min(adjusted * 0.18, 54.0)
            accumulator.match_reasons.append("step39 понижение: другая ЧС-выплата, вопрос про материальную помощь")
        elif title_is_tjs_material_help:
            adjusted = min(adjusted * 0.22, 58.0)
            accumulator.match_reasons.append("step39 понижение: явный ЧС-вопрос, не общая ТЖС")

    if chs_property_question_v2:
        if title_is_chs_property:
            adjusted = max(adjusted + 150.0, 140.0)
            accumulator.match_reasons.append("step39 усиление: утрата имущества при ЧС")
        elif title_is_chs_material_help or title_is_chs_health or title_is_chs_death:
            adjusted = min(adjusted * 0.18, 54.0)
            accumulator.match_reasons.append("step39 понижение: другая ЧС-выплата, вопрос про утраченное имущество")
        elif title_is_tjs_material_help:
            adjusted = min(adjusted * 0.22, 58.0)
            accumulator.match_reasons.append("step39 понижение: явный ЧС-вопрос про имущество, не общая ТЖС")

    if chs_health_question_v2:
        if title_is_chs_health:
            adjusted = max(adjusted + 150.0, 140.0)
            accumulator.match_reasons.append("step38 усиление: вред здоровью при ЧС")
        elif title_is_chs_material_help or title_is_chs_property or title_is_chs_death:
            adjusted = min(adjusted * 0.18, 54.0)
            accumulator.match_reasons.append("step38 понижение: другая ЧС-выплата, вопрос про вред здоровью")
        elif title_is_tjs_material_help:
            adjusted = min(adjusted * 0.22, 58.0)
            accumulator.match_reasons.append("step38 понижение: явный ЧС-вопрос про вред здоровью, не общая ТЖС")

    if chs_death_question_v2:
        if title_is_chs_death:
            adjusted = max(adjusted + 150.0, 140.0)
            accumulator.match_reasons.append("step38 усиление: семья погибшего/умершего при ЧС")
        elif title_is_chs_material_help or title_is_chs_property or title_is_chs_health:
            adjusted = min(adjusted * 0.18, 54.0)
            accumulator.match_reasons.append("step38 понижение: другая ЧС-выплата, вопрос про погибшего/умершего")
        elif title_is_tjs_material_help:
            adjusted = min(adjusted * 0.22, 58.0)
            accumulator.match_reasons.append("step38 понижение: явный ЧС-вопрос про погибшего/умершего, не общая ТЖС")

    child_disabled_equipment_question_v2 = (
        question_has_child_context
        and ("инвалид" in question_tokens or q_text_has_any({"ребенок инвалид", "ребенка инвалид", "ребенку инвалид", "детей инвалид"}))
        and q_text_has_any({"коляск", "кресло", "каталк", "слухов", "аппарат"})
    )
    title_is_child_disabled_equipment = (
        service_title_has_any({"дет", "ребен"})
        and service_title_has_any({"инвалид"})
        and service_title_has_any({"коляск", "кресло", "каталк", "слухов", "аппарат"})
    )
    if child_disabled_equipment_question_v2:
        if title_is_child_disabled_equipment:
            adjusted = max(adjusted + 150.0, 140.0)
            accumulator.match_reasons.append("step38 усиление: кресло-каталка/слуховой аппарат ребёнку-инвалиду")
        elif service_title_has_any({"тср", "техническ", "реабилитац", "транспорт", "автомоб"}) or (
            service_title_has_any({"инвалид"}) and not service_title_has_any({"дет", "ребен"})
        ):
            adjusted = min(adjusted * 0.16, 52.0)
            accumulator.match_reasons.append("step38 понижение: не профильная детская АМП на средство реабилитации")

    jku_question_v2 = utility_question or q_text_has_any({
        "жку",
        "жкх",
        "коммуналк",
        "коммунальн",
    ...
    broad_jku_without_category_or_territory_v2 = (
        jku_question_v2
        and not q_raw_has_any({"эвенки", "эвенкийск", "таймыр", "сво", "боев", "вов", "ветеран", "инвалид", "многодетн", "пенсионер", "донор"})
    )
    title_is_jku_service = (
        service_title_has_any({"жку", "жкх", "коммун"})
        or (
            service_title_has_any({"субсид", "компенсац", "мсп"})
            and service_title_has_any({"оплат"})
            and service_title_has_any({"жил"})
        )
    )
    title_is_repair_service = (
        service_title_has_any({"ремонт"})
        or service_text_has_any({"ремонт жилого помещения", "текущий ремонт"})
    )
    if jku_question_v2:
        if title_is_jku_service:
            adjusted = max(adjusted + 78.0, 96.0)
            accumulator.match_reasons.append("step38 усиление: вопрос про оплату жилья и коммунальных услуг")
            if broad_jku_without_category_or_territory_v2:
                accumulator.match_reasons.append("step39 ambiguity_guard: широкий ЖКУ-вопрос без категории/территории")
        if q_has_any({"эвенки", "эвенкийск"}):
            if title_is_jku_service and service_title_has_any({"эвенки"}):
                adjusted = max(adjusted + 120.0, 130.0)
                accumulator.match_reasons.append("step38 усиление: ЖКУ/компенсация в Эвенкии")
            elif not service_title_has_any({"эвенки"}):
                adjusted = min(adjusted * 0.42, 68.0)
                accumulator.match_reasons.append("step38 понижение: вопрос про ЖКУ в Эвенкии, услуга не Эвенкия")
        if title_is_repair_service and not q_text_has_any({"ремонт", "ремонтир", "отремонт"}):
            adjusted = min(adjusted * 0.08, 34.0)
...
>>>>>>> 4c04853102b91a0c6e1fdd3692b3fb98688e2646
    ...
    # не содержит достаточной категории/территории для выбора одной услуги.
    # В step39 верхние ремонтные услуги уже были убраны, но resolver всё ещё
    # мог уверенно выбрать одну территориальную ЖКУ-услугу по большому отрыву.
    # Для таких вопросов нужен уточняющий ответ, если есть несколько ЖКУ-кандидатов.
    if _candidate_has_broad_jku_guard(first):
...
def _compact_match_reasons_for_candidate(match_reasons: list[str], *, limit: int = 12) -> list[str]:
    """
    Keep debug reasons compact, but never drop resolver control markers.

    Some later ranking rules add service-level guard reasons that are consumed by
    _choose_resolution_status(). If those reasons are appended after many exact
    alias matches, a naive match_reasons[:10] truncation hides the guard from the
    chooser and the resolver may incorrectly return resolved instead of
    ambiguous.
    """
    ...
    control_reasons = [
        ...
        if "ambiguity_guard:" in reason or "resolution_guard:" in reason
    ]
    regular_reasons = [
        reason
        for reason in match_reasons
    ...
    for reason in control_reasons + regular_reasons:
        if reason in seen:
            continue
