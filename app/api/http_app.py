...
def _build_llm_answer_composer_service(settings: AppSettings) -> Any | None:
    ...
    Disabled by default. In second_step_48 it is intended for runtime shadow
    diagnostics only: it writes diagnostics into debug/answer_payload_json and
    does not replace the answer sent to users.
    """
    config = settings.llm_answer_composer
    ...
    client = OpenAIClientFactory(settings.openai).create_async_client()
    return LLMAnswerComposerService(
        client,
